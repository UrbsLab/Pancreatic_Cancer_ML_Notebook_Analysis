...
"""
def dataScaling(td, outcomeLabel, instLabel, matchName):
    #Convert data to scikit learn format
    y_data = td[outcomeLabel]
    if instLabel != None and instLabel != 'None':
        inst_data = td[instLabel] #pull out instance labels in case they include text
    if matchName != None and instLabel != 'None':
        match_data = td[matchName]
    
    if instLabel == None or instLabel == 'None':
        if matchName == None or matchName == 'None':
            x_data = td.drop([outcomeLabel], axis=1)
        else:
            x_data = td.drop([outcomeLabel,matchName], axis=1)
    else:
        if matchName == None or matchName == 'None':
            x_data = td.drop([outcomeLabel,instLabel], axis=1)
        else:
            x_data = td.drop([outcomeLabel,instLabel,matchName], axis=1)
    ...
    if instLabel == None or instLabel == 'None':
        if matchName == None or matchName == 'None':
            td = pd.concat([pd.DataFrame(y_data), pd.DataFrame(x_data_scaled)], axis=1, sort=False)
        else:
            td = pd.concat([pd.DataFrame(y_data), pd.DataFrame(match_data), pd.DataFrame(x_data_scaled)], axis=1, sort=False)
    else:
        if matchName == None or matchName == 'None':
            td = pd.concat([pd.DataFrame(y_data), pd.DataFrame(inst_data), pd.DataFrame(x_data_scaled)], axis=1, sort=False)
        else:
            td = pd.concat([pd.DataFrame(y_data), pd.DataFrame(inst_data), pd.DataFrame(match_data), pd.DataFrame(x_data_scaled)], axis=1, sort=False)
    return td
"""
"""
def dataScaling(train_dfs, test_dfs, outcomeLabel, instLabel, name_path):
